...
// int wrapper_getcurx(WINDOW* win) { return getcurx(win); }
// int wrapper_getcury(WINDOW* win) { return getcury(win); }
...
// Get cursor position
func (window *Window) Getyx() (y, x int) {
	y = int(C.wrapper_getcury(window.cwin))
	x = int(C.wrapper_getcurx(window.cwin))
	return y, x
}

// Move cursor in window
func (window *Window) Wmove(y, x int) {
	C.wmove(window.cwin, C.int(y), C.int(x))
}

// Delete character at cursor
func (window *Window) Wdelch() {
	C.wdelch(window.cwin)
}
